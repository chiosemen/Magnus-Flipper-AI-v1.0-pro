/**
 * Craigslist Cron Endpoint
 * POST /api/cron/craigslist
 * Triggered by Vercel Cron to crawl Craigslist
 */
import { NextRequest, NextResponse } from 'next/server';
import { verifyCronSecret } from '@/lib/auth';
import { getSupabaseAdmin } from '@/lib/db';
import { crawlCraigslist } from '@/lib/crawlers';
  ...
  try {
    ...
    if (!savedSearches || savedSearches.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'No active Craigslist searches found',
        results,
    ...
    for (const search of savedSearches) {
      try {
        ...
        const crawlResult = await crawlCraigslist(crawlerParams);
