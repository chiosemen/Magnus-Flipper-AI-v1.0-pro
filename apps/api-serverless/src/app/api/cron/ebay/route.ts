 ...
 * eBay Cron Endpoint
 * POST /api/cron/ebay
 * Triggered by Vercel Cron to crawl eBay local pickup listings
 */
import { NextRequest, NextResponse } from 'next/server';
import { verifyCronSecret } from '@/lib/auth';
import { getSupabaseAdmin } from '@/lib/db';
import { crawlEbay } from '@/lib/crawlers';
  ...
  try {
    ...
    if (!savedSearches || savedSearches.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'No active eBay searches found',
        results,
    ...
    for (const search of savedSearches) {
      try {
        ...
        const crawlResult = await crawlEbay(crawlerParams);
