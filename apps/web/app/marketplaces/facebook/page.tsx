...
function relativeTimeLong(ts: number, nowMs: number | null) {
  if (typeof nowMs !== "number" || !Number.isFinite(nowMs)) return "just now";
...
export default function FacebookMarketplacePage() {
  ...
  const [dealsLastUpdatedAt, setDealsLastUpdatedAt] = useState<
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/facebook/deals/last-updated?region=${encodeURIComponent(region)}`
        );
        if (!res.ok) {
          setDealsLastUpdatedAt(null);
          return;
        }
        const payload = await res.json();
        ...
        if (typeof lastUpdatedAt === "string" && lastUpdatedAt.length > 0) {
          ...
          setDealsLastUpdatedAt(Number.isFinite(parsed) ? parsed : null);
          return;
        }
        setDealsLastUpdatedAt(null);
      } catch (error) {
        console.warn("Deals last-updated load skipped", error);
        setDealsLastUpdatedAt(null);
      }
    })();
  }, [region]);
  ...
  const loadDealsForSearch = async (searchId: string) => {
    ...
    try {
      ...
      setSelectedSearchDealsError(error?.message || "Failed to load matched deals");
    } finally {
        ...
        <section id="saved-searches" className="py-12 bg-[#0A0A0A]">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
              ...
              {!authLoading && user && !loadingSavedSearches && savedSearches.length === 0 && (
                <div className="mt-6 bg-[#121212] border border-white/10 rounded-xl p-6 text-center">
                  <p className="text-white/80 font-semibold">No saved searches yet</p>
                  <p className="text-white/60 text-sm mt-1">
                    Create a search above to start monitoring new deals automatically.
                  </p>
                </div>
              )}
                  ...
                  {savedSearches.map((search) => {
                    ...
                    return (
                        ...
                        onClick={() => {
                          ...
                          if (search.marketplace === "facebook") {
                          ...
                          {isCarSearch ? (
                            <>
                              ...
                              {typeof carSummary.maxPrice === "number" && (
                                <span className="px-2 py-1 bg-white/5 border border-white/10 rounded-full text-white/70">
                                  Max price: {carSummary.maxPrice.toLocaleString()}
                                </span>
                              )}
                            </>
                          ) : (
                                  ...
                                  {selectedSearchDeals.map((deal) => {
                                    ...
                                    const rawPrice =
                                      typeof dealPrice === "number"
                                        ...
                                        : null;
                                    const location =
                                      deal.location || data?.location || data?.city;
                                    const url = deal.url || data?.url || data?.link;
                                      ...
                                      >
                                        ...
                                        <div className="flex items-center justify-between mt-2 text-xs text-white/70">
                                          ...
                                          <span className="text-white/50">
                                            {deal.created_at
                                              ? timeAgo(new Date(deal.created_at).getTime(), now)
                                              : ""}
                                          </span>
                                        </div>
                                        {isCarSearch ? (
                                          <>
                                            <div className="text-xs text-white/60 mt-1 line-clamp-1">
                                              {[
                                                (() => {
                                                  const year = parseNumericFilter(data?.year);
                                                ...
                                                (() => {
                                                  const mileage = parseNumericFilter(
                                                    data?.mileage ?? data?.odometer ?? data?.miles
                                                  );
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-6">
              <div className="flex items-start justify-between gap-3">
                <div>
                  {dealsLastUpdatedAt !== undefined && (
                    <div className="text-xs text-white/50 font-medium mb-2">
                      {dealsLastUpdatedAt
                        ? `Updated ${relativeTimeLong(dealsLastUpdatedAt, now)}`
                        : "No updates yet"}
                    </div>
                  )}
                  <h2 className="text-2xl font-extrabold text-white mb-2 tracking-tight">
