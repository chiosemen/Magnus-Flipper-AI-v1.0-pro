...
export function FeedCard({ title, children, price, marketValue, profit, roi, marketplace, timestamp }: FeedCardProps) {
  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] hover:border-[#3a3a3a] rounded-lg p-4 transition-smooth hover-lift cursor-pointer">
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-[#ededed] mb-1.5 line-clamp-2">{title}</h3>
          <div className="flex items-center gap-3 text-xs text-[#a0a0a0]">
            {marketplace && <span className="capitalize font-medium">{marketplace}</span>}
      ...
      {price !== undefined && marketValue !== undefined && profit !== undefined && roi !== undefined && (
        <div className="space-y-1.5 pt-2 border-t border-[#2a2a2a]">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-semibold text-[#ededed]">£{price}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#a0a0a0]">Market Value</span>
            <span className="text-sm font-semibold text-[#ededed]">£{marketValue}</span>
          </div>
          <div className="flex items-center justify-between pt-1">
            <span className="text-xs text-[#a0a0a0]">Est. Profit</span>
            <span className="text-sm font-bold text-green-500">£{profit}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#a0a0a0]">ROI</span>
            <span className="text-sm font-bold text-green-500">{roi}%</span>
          </div>
        </div>
      )}
    </div>
