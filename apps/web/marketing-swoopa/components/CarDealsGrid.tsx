...
import { CarDealCard } from "./deals/CarDealCard";
...
function CarDealCardItem({
  ...
  return (
    <motion.div
      ...
      {...debug}
    >
      <CarDealCard deal={deal} tier={tier} motion={motionSeverity} />
    </motion.div>
  );
}

export default function CarDealsGrid({
  ...
  const debugSkeleton = useMotionDebug({
    label: "CarDealsGrid:Skeleton",
    type: "transition",
    durationMs: motionSeverity.baseDurationMs,
    tier: motionSeverity.tier,
  });
  const debugDeals = useMotionDebug({
    label: "CarDealsGrid:Deals",
    type: "transition",
    durationMs: motionSeverity.baseDurationMs,
    tier: motionSeverity.tier,
  });
          ...
          >
            ...
            <div className="columns-2 md:columns-3 lg:columns-4 gap-4 [column-fill:_balance]">
              {Array.from({ length: skeletonCount }).map((_, idx) => (
                <SkeletonDealCard key={`skeleton-${idx}`} variant="landscape" />
              ))}
            </div>
          </motion.div>
        ) : (
