 ...
 */
function buildListingsQuery(filter) {
 ...
 */
function applyInMemoryFilters(listings, filter) {
    // Apply radius filter if lat/lng provided
    if (filter.locationLat !== undefined &&
        filter.locationLng !== undefined &&
        filter.radiusMiles !== undefined) {
        return listings.filter((listing) => {
            if (!listing.latitude || !listing.longitude)
                return false;
            const distance = haversineDistance(filter.locationLat, filter.locationLng, listing.latitude, listing.longitude);
 ...
 */
async function processSavedSearch(search) {
    ...
    console.log(`Found ${matchedListings.length} matched listings (from ${candidateListings.length} candidates)`);
 ...
 * Main job execution
 */
async function runSavedSearchJob() {
...
//# sourceMappingURL=savedSearchJob.js.map