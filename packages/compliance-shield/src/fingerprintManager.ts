 ...
 */
export function getFingerprintWithMutation(
  ...
  let selected: FingerprintCache | null = null;
 ...
 */
function mutateFingerprint(base: RequestFingerprint): RequestFingerprint {
  const mutated = { ...base };
 ...
 */
export function clearFingerprintCache(marketplace?: MarketplaceId): void {
  if (marketplace) {
 ...
 */
export function getFingerprintStats(): Record<string, { count: number; avgUseCount: number }> {
  const stats: Record<string, { count: number; avgUseCount: number }> = {};
