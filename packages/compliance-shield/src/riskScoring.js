 ...
 */
function jsChallengeRiskToScore(jsChallengeRisk) {
 ...
 */
function throttleBudgetToScore(throttleBudget) {
    if (throttleBudget === 0)
        return 0; // Unlimited = no risk
    if (throttleBudget < 1000)
        return 80; // Very restrictive
    if (throttleBudget < 5000)
        return 60; // Restrictive
    if (throttleBudget < 10000)
        return 40; // Moderate
    if (throttleBudget < 20000)
        return 20; // Permissive
 ...
 */
function antiBotRequirementsToScore(profile) {
    let score = 0;
    if (profile.requiresUserAgentRotation)
        score += 15;
    if (profile.requiresProxyRotation)
        score += 25;
    if (profile.requiresCookieSession)
        score += 20;
 ...
 */
export function calculateRiskScore(profile, historicalBlockRate) {
    const factors = {
    ...
    let overall = factors.riskLevel * weights.riskLevel +
 ...
 */
export function compareRiskScores(scores) {
    return scores
