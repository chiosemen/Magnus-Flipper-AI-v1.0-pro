 ...
 */
export async function recordSearchRun(metrics) {
    try {
        ...
        console.log(`[Analytics] Recorded run for search ${metrics.searchId}: ${metrics.listingsScanned} scanned, ${metrics.matchesFound} matches`);
    }
    catch (error) {
        console.error(`[Analytics] Failed to record search run for ${metrics.searchId}:`, error);
    }
}
/**
 * Get comprehensive stats for a single search
 */
export async function getSearchStats(searchId) {
    try {
        ...
        const now = new Date();
        const daysSinceCreation = Math.max(1, Math.floor((now.getTime() - search.createdAt.getTime()) / (1000 * 60 * 60 * 24)));
 ...
 */
export async function getUserSearchStats(userId) {
    try {
 ...
 */
export async function getSearchActivityTimeline(searchId, limit = 20) {
    try {
        ...
        return alerts.map((alert) => ({
            ...
            url: alert.url,
            imageUrl: alert.metadata?.imageUrl,
        }));
    }
    catch (error) {
        console.error(`[Analytics] Failed to get activity timeline for search ${searchId}:`, error);
        return [];
    }
}
/**
 ...
 */
export async function getUserAggregatedStats(userId) {
    try {
        const searches = await prisma.savedSearch.findMany({
            where: { userId },
            select: {
        ...
        searches.forEach((search) => {
            const days = Math.max(1, Math.floor((now.getTime() - search.createdAt.getTime()) / (1000 * 60 * 60 * 24)));
            totalDays += days;
        });
