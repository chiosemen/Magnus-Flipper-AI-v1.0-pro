/**
 * Shared domain types for Magnus Flipper AI search functionality
...
export interface SearchFilter {
  category: string; // "Phones", "Cars", etc.
  manufacturer?: string; // "Apple", "Samsung"
  models?: string[]; // ["iPhone 15 Pro Max", ...]
  minPrice?: number;
  maxPrice?: number;
  radiusMiles?: number;
  locationCity?: string;
  ...
  sites?: MarketplaceSite[]; // ["OFFERUP", "CRAIGSLIST"]
}

export interface SavedSearch extends SearchFilter {
  id: string;
  userId: string;
  name: string; // "NYC iPhone 15 flips"
  maxResultsPerRun: number; // to avoid spam
  active: boolean;
  createdAt: string;
  updatedAt: string;
...
export interface AlertsRecentResponse {
  matches: Array<ListingMatch & { listing: Listing; savedSearch: SavedSearch }>;
  total: number;
}
