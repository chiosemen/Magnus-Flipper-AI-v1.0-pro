...
export class ScraperMonitor {
     ...
     */
    async updateHealthMetrics(result) {
        ...
        if (existing) {
            ...
            const avgItems = (existing.avg_items_per_run * existing.total_runs +
                result.total_scraped) /
                totalRuns;
            const avgDuration = (existing.avg_duration_ms * existing.total_runs + result.duration_ms) /
                totalRuns;
     ...
     */
    async getHealthMetrics(marketplace) {
        const { data, error } = await this.supabase
            .from("scraper_health")
            .select("*")
     ...
     */
    async getRecentLogs(marketplace, limit = 50) {
    ...
    async getPerformanceStats(marketplace) {
        ...
        const avgDuration = successful.reduce((sum, log) => sum + log.duration_ms, 0) /
            successful.length || 0;
        const avgScraped = successful.reduce((sum, log) => sum + log.total_scraped, 0) /
            successful.length || 0;
     ...
     */
    async checkStaleScrapers(hoursStale = 24) {
        const cutoffTime = new Date(Date.now() - hoursStale * 60 * 60 * 1000).toISOString();
        const { data, error } = await this.supabase
            .from("scraper_health")
...
//# sourceMappingURL=scraperMonitor.js.map