...
import { getTierLimits, getUserTier } from "./tier-config.js";
/**
 ...
 */
export async function canCreateSearch(userId) {
 ...
 */
export async function canReceiveAlert(userId) {
    const userData = await getUserWithTier(userId);
    if (!userData) {
        return { allowed: false, reason: "User not found" };
    }
 ...
 */
export async function canAccessMarketplace(userId, marketplace) {
    const userData = await getUserWithTier(userId);
    if (!userData) {
        return { allowed: false, reason: "User not found" };
    }
 ...
 */
export async function canReceiveEmailAlerts(userId) {
    const userData = await getUserWithTier(userId);
    if (!userData) {
        return { allowed: false, reason: "User not found" };
    }
