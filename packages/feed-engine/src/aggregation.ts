...
import { rankListings, type ListingForRanking, type RankingScore } from "./ranking";
import { deduplicateListings, generateFingerprint, type ListingForFingerprint } from "./fingerprint";
...
export function aggregateListings(
  listings: Array<ListingForRanking & ListingForFingerprint>,
  ...
  const fingerprintCache = new Map<string, ReturnType<typeof generateFingerprint>>();
  ...
  const byFingerprint = new Map<string, typeof processed>();
  for (const listing of processed) {
  ...
  return paginated as AggregatedListing[];
}

/**
 * Calculate marketplace average prices
 */
export function calculateMarketplaceAvgPrices(
