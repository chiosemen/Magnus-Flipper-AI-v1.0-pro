...
export interface RankingScore {
  listingId: string;
  velocityScore: number; // 0-100, based on time since first seen
  freshnessScore: number; // 0-100, based on recency
  priceScore: number; // 0-100, based on price competitiveness (lower = better)
  engagementScore: number; // 0-100, based on views/interactions
 ...
 */
export function calculateVelocityScore(listing: ListingForRanking): number {
  const now = new Date();
 ...
 */
export function calculateFreshnessScore(listing: ListingForRanking): number {
  const now = new Date();
  const lastSeen = listing.lastSeen instanceof Date ? listing.lastSeen : new Date(listing.lastSeen);
  const hoursSinceLastSeen = (now.getTime() - lastSeen.getTime()) / (1000 * 60 * 60);
  ...
  if (hoursSinceLastSeen > 168) return 0; // 7 days
 ...
 */
export function calculateEngagementScore(listing: ListingForRanking): number {
  ...
  if (views < 10) return 90;
  if (views < 50) return 70;
  if (views < 100) return 50;
  if (views < 500) return 30;
...
): Array<ListingForRanking & { rankingScore: RankingScore }> {
