...
function validateRequirement(
  ...
  if (requirement.includes('cite scrape_runs + anomalies')) {
    return (
      result.evidence?.runs?.length >= 0 && result.evidence?.anomalies?.length >= 0
    );
  }
  ...
  if (requirement.includes('NOT mention selectors unless evidence')) {
    ...
    const hasEvidence = result.evidence?.anomalies?.some(
      (a: any) => a.type === 'ZERO_RESULTS'
    );
  ...
  if (requirement.includes('explicitly state missing telemetry')) {
    const text = JSON.stringify(result).toLowerCase();
    return (
      /missing|insufficient|no data|no telemetry|lack of/.test(text) &&
