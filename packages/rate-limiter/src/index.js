...
if (!process.env.REDIS_URL) {
    console.warn('[rate-limiter] REDIS_URL is not set. Rate limiting will be disabled (all requests allowed).');
}
const redis = process.env.REDIS_URL
    ...
    : null;
function buildKey(parts, suffix) {
 ...
 */
export async function tryConsume(parts, tokens = 1) {
 ...
 */
export async function registerBackoff(parts) {
    if (!redis) {
        const profile = getMarketplaceProfile(parts.marketplace);
    ...
    const next = Math.min(existing * profile.backoffMultiplierOn429, profile.exponentialBackoffMaxSeconds);
    ...
    const jitteredNext = Math.max(profile.recommendedPingIntervalSeconds, Math.floor(next + baseJitter + exponentialJitter));
    await redis.set(backoffKey, String(jitteredNext), 'EX', profile.cooldownSecondsOn429);
    return jitteredNext;
}
/**
 * Get current backoff interval with jitter applied.
 */
export async function getCurrentBackoffSeconds(parts) {
    const profile = getMarketplaceProfile(parts.marketplace);
    if (!redis) {
 ...
 */
export async function getAdaptiveThrottleMultiplier(parts, lookbackMinutes = 15) {
    if (!redis)
        return 1.0;
    const profile = getMarketplaceProfile(parts.marketplace);
    const baseKey = buildKey(parts);
 ...
 */
export async function recordRequestOutcome(parts, success, lookbackMinutes = 15) {
    if (!redis)
        return;
    const baseKey = buildKey(parts);
    const key = success
