 ...
 */
export function loadCookieHeader(filename: string): string {
  ...
  const cookies: CookieData[] = JSON.parse(fs.readFileSync(fullPath, 'utf8'));
 ...
 */
export function loadCookiesForPlaywright(filename: string, domain: string): CookieData[] {
  // Resolve path relative to this file's location (works in both src/ and dist/)
  // From runtime/cookies.ts -> dev/ = ../dev/
  // From dist/runtime/cookies.js -> dev/ = ../../dev/
  const fullPath = path.resolve(__dirname, '../../dev', filename);
