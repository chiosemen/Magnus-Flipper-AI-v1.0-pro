...
import { loadCookieHeader, loadCookiesForPlaywright, type CookieData } from './cookies.js';

// Lazy load cookies to avoid errors if files don't exist
let cookiesLoaded = false;
const cookieCache: Record<string, string> = {};
const playwrightCookieCache: Record<string, CookieData[]> = {};
...
export const MARKETPLACE_COOKIES: Record<string, string> = new Proxy({} as Record<string, string>, {
...
export const MARKETPLACE_COOKIES_PLAYWRIGHT: Record<string, CookieData[]> = new Proxy({} as Record<string, CookieData[]>, {
  get(target, prop: string) {
    if (!cookiesLoaded) {
      loadCookies();
    }
