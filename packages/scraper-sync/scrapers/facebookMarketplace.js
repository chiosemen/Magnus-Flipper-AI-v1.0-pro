...
export class FacebookMarketplaceScraper {
    ...
    async scrape() {
        ...
        try {
            ...
            if (this.config.auth_credentials?.email &&
    ...
    async login(page) {
        try {
            ...
            // Enter password
            await page.fill('input[name="pass"]', this.config.auth_credentials.password);
            await this.browserManager.randomDelay(500, 1000);
            // Click login
     ...
     */
    async infiniteScrollLoadItems(page) {
        let previousCount = 0;
        let scrollAttempts = 0;
        const maxScrolls = this.config.max_pages * 3;
        while (scrollAttempts < maxScrolls) {
            // Get current item count
            const currentCount = await page.$$eval('div[class*="x9f619"]', (elements) => elements.length);
