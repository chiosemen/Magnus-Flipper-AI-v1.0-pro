...
export class VintedScraper {
    ...
    API_URL = "https://www.vinted.com/api/v2";
    sessionCookie = null;
    constructor(config) {
     ...
     */
    async fetchListingsFromAPI(query, page) {
        ...
        try {
            const response = await axios.get(`${this.API_URL}/catalog/items?${params.toString()}`, { headers });
            const items = response.data?.items || [];
            return items.map((item) => this.transformAPIItem(item));
        }
        catch (error) {
...
//# sourceMappingURL=vinted.js.map