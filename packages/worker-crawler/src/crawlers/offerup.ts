import { scrape } from "@magnus-flipper-ai/offerup-crawler";
...
export async function crawl(filter: SearchFilter): Promise<ScrapedListing[]> {
  ...
  if (result.warning) {
    console.warn('[OfferUp]', result.warning);
  }

  return result.listings.map((listing) => ({
    marketplace: "OFFERUP" as const,
...
function buildSearchParams(filter: SearchFilter) {
  ...
  if (filter.locationLat && filter.locationLng) {
    params.location = {
