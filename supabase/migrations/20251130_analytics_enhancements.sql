...
-- Adds tables for price history, performance tracking, and conversion metrics
...
CREATE INDEX idx_price_history_marketplace_external_id ON price_history(marketplace, external_id);
...
SELECT
  ...
  FIRST_VALUE(ph.price) OVER (PARTITION BY ph.marketplace, ph.external_id ORDER BY ph.recorded_at DESC) as current_price,
  FIRST_VALUE(ph.price) OVER (PARTITION BY ph.marketplace, ph.external_id ORDER BY ph.recorded_at ASC) as initial_price,
  (FIRST_VALUE(ph.price) OVER (PARTITION BY ph.marketplace, ph.external_id ORDER BY ph.recorded_at DESC) -
   FIRST_VALUE(ph.price) OVER (PARTITION BY ph.marketplace, ph.external_id ORDER BY ph.recorded_at ASC)) as total_price_change,
  MIN(ph.recorded_at) as first_seen,
  MAX(ph.recorded_at) as last_seen
FROM price_history ph
JOIN marketplace_listings ml ON ph.listing_id = ml.id
GROUP BY ph.marketplace, ph.external_id, ml.title, ml.url;
